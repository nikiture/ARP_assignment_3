...
int main (int argc, char ** argv) {
    ...
    fd_set log_check;
    const int proc_numb = 3; //0 for client, 1 for obstacle, 2 for target
    ...
    while (1) {
        ...
        for (count = 0; count < proc_numb; count ++) { 
            ...
            if (proc_info [count] [0] == 'q') { //quit message from client process
                printf ("watchdog here! received from client process quit message, ending the monitoring of the processes\n");
                sleep (1);
