...
int process_request (int, int, struct position *, struct position [obs_num], struct position [targ_num], char [1024], char [1024], int *, int *, int *, int *, int *);
...
int main (int argc, char* argv[] ) {
    ...
    int fd_in [pipe_numb]; //0 for dynamics, 1 for map + kb, 2 and 3 for remote server (connecting to generators processes on others machines); first column for requests, second column for server's answer
    int fd_out [pipe_numb];
    //index 2 refers to a virtual obstacle generator while being a pipe to the socket server process
    //same logic for index 3 and target generator process
    ...
    char request_string [1024], answer_string [1024], start_str [1024];
    ...
    while (read (fd_in [2], request_string, 1024) < 0) {
        if (errno != EINTR) {
    ...
    while (read (fd_in [3], request_string, 1024) < 0) {
        if (errno != EINTR) {
            perror ("target read");
            exit (EXIT_FAILURE);
        }
    ...
    while (1) {
        ...
        switch (write_req) {
            ...
            default: //more than one process has pending request, random choice for fairness after identifying requesting processes
                ...
                syscall_res = read (fd_in [req_idx], request_string, 1024);
                if (syscall_res < 0) {
                    perror ("read");
        ...
        if (reset == 1) { //received reset instruction from the keyboard process
            ...
            printf ("BB: sending reset message to the generating processes\n");
            targ_to_reach = 0;
            reset = 0;
            printf ("BB: waiting for osbtacle reception\n");
            while (read (fd_in [2], request_string, 1024) < 0) {
                if (errno != EINTR) {
                    perror ("obstacles read");
                    exit (EXIT_FAILURE);
            ...
            printf ("BB: waiting for target reception\n");
            while (read (fd_in [3], request_string, 1024) < 0) {
                if (errno != EINTR) {
                    perror ("target read");
                    exit (EXIT_FAILURE);
            ...
            printf ("BB: new obstacles and target stored\nsending aknowledgement to map process\n");
            if (sprintf (answer_string, "%c", 'o') < 0) {
                perror ("map formating");
                exit (EXIT_FAILURE);
            }
            if (write (fd_out [1], answer_string, strlen (answer_string) + 1) < 0) {
...
int process_request (int req_idx, int request_fd_o, struct position * drone_pos, struct position obs_pos [obs_num], struct position targ_pos [targ_num], char pipe_msg [1024], char out_msg [1024], int * changed_obstacles, int * obs_placed, int changed_targets [2], int * targ_placed, int * curr_targ) {
    char tmp_msg [1024];
    switch (req_idx) {
        case 0: //this is drone dynamic: it can send the updated drone position or request the obtacle position
            ...
            if (pipe_msg [0] == 't') {//process requesting target position
                ...
                //printf ("new currrent target received, %d\n", *curr_targ);
        ...
        case 1: //map displayer: can request drone or obstacle positions or write to signal a reset
            if (pipe_msg [0] == 'd') {//requested drone position
                ...
                //printf ("the current target to reach is the number %d at position %lf %lf\n", *curr_targ, targ_pos [*curr_targ].x, targ_pos [*curr_targ].y);
