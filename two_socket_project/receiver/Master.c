...
int main (int argc, char ** argv) {
    ...
    const int proc_numb = 5;
    int null_wait;
    int logfd [proc_numb - 1];
    ...
    printf ("%d %d\n%d %d\n", out_fds [3] [0], out_fds [3] [1], in_fds [3] [0], in_fds [3] [1]);
    ...
    if (proc_pid [0] == 0) {
        char * arglist1 [] = {/*"konsole", "-e", */"./drone_dyn", process_fd [0], kb_din_fd, NULL};
    ...
    if (proc_pid [1] == 0) {
        char * arglist2 []= {"konsole", "-e", "./map_displayer", process_fd [1], kb_din_fd, NULL};
    ...
    if (proc_pid [3] == 0) {
        char * obsarglist [] = {"./server_receiver", argv [1], process_fd [2], process_fd [3], NULL};
    ...
    if (proc_pid [4] == 0) {
        char * argwtcdg [] = {"./watchdog", kons_map_pid, NULL}; //sending to watchdog also pids of konsole to properly close it in case of crash
    ...
    term_child = waitpid (proc_pid [4], &null_wait, 0);
